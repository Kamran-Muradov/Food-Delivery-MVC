$(function () {
    ...
    $("#form-create").validate({
        ...
        submitHandler: function (form) {
            ...
            let data = JSON.stringify(
                {
                    discount: $('#table-area #form-create #discount').val(),
                    expiryDate: $('#table-area #form-create #expiryDate').val()
                })
                ...
                error: function (xhr, status, error) {
                    ...
                    if (xhr.status == 409) {
                        ...
                        Swal.fire({
                            icon: "error",
                            title: "Oops...",
                            text: "Country with this name already exists",
                        });
                    } else {
                        $("#form-create #create-btn").removeClass("d-none")
                        $("#form-create #loading-create-btn").addClass("d-none")
    ...
    $(document).on('click', '#table-area .table-tbody .btn-warning', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#table-area #modal-edit #discount').val(response.discount)
    ...
    $("#form-edit").validate({
        ...
        submitHandler: function (form) {
            ...
            let data = JSON.stringify(
                {
                    discount: $('#table-area #modal-edit #discount').val(),
                    expiryDate: $('#table-area #modal-edit #expiryDate').val(),
            ...
            $.ajax({
                ...
                success: function () {
                    ...
                    let discount = $('#table-area #modal-edit #discount').val()
                    let expiryDate = $('#table-area #modal-edit #expiryDate').val()
                    let status = $('#table-area #modal-edit #status').val() === 'true' ? "Active" : "Inactive"
                    ...
                    row.find('#expiry-date').html(formattedDate)
    ...
    function updateTable(response) {
        ...
        $.each(response.datas, function (index, item) {
            ...
            html += `<tr data-id="${item.id}">
                                    <td class="sort-name">${item.code}</td>
                                    <td id="discount" class="sort-name">${item.discount}%</td>
                                    <td id="status" class="sort-name">${item.isActive ? "Active" : "Inactive"}</td>
                                    <td id="expiry-date" class="sort-name">${formattedDate}</td>
